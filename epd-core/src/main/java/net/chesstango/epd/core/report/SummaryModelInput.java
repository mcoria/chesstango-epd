...
public record SummaryModelInput(List<EpdSearchResult> epdSearchResults,
    ...
    public static SummaryModelInput load(String suiteName, List<EpdSearchResult> epdSearchResults) {
        ...
        NodesVisitedModel nodesReportModel = new NodesVisitedModel().collectStatistics(suiteName, epdSearchResults.stream().map(EpdSearchResult::getSearchResult).toList());
        ...
        return new SummaryModelInput(epdSearchResults, epdSearchModel, boardModel, nodesReportModel, evaluationReportModel, principalVariationReportModel, transpositionReportModel);
    }
}
